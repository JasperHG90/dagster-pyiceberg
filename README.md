...
| Sort order | ❌ | https://shorturl.at/TycZN | Currently not supported in PyIceberg. Sort ordering is supported when creating a table from an Iceberg schema (one must pass the source_id which can be inferred from a PyArrow schema but this is shaky). However, one cannot simply update a sort ordering like a partition or schema spec. |
